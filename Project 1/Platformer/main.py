...
class Platformer:
    ...
    def load_data(self):
        ...
        self.jumped_on_enemy = pg.mixer.Sound(path.join(self.snd_dir, "jumpOnEnemy.wav"))
    ...
    def update(self):
        ...
        if mob_hits:
            for mob in mob_hits:
                if mob.rect.y-1 > self.player.rect.y and (self.player.rect.x > mob.rect.x-mob.rect.width / 2 - 8) and (self.player.rect.x < mob.rect.x + mob.rect.width/2 + 8):
                    self.jumped_on_enemy.play()
                    self.player.vel.y = -BOOST_POWER * (3/5)
            ...
            if hits:
                ...
                for hit in hits:
                    if hit.rect.bottom > lowest.rect.bottom:
                        lowest = hit
                if self.player.pos.x < lowest.rect.right+12 and self.player.pos.x > lowest.rect.left-12:
    ...
    def wait_for_key(self):
        ...
        while waiting:
            ...
            for event in pg.event.get():
                ...
                if event.type == pg.KEYDOWN:
                    keydown = True
                if keydown == True and event.type == pg.KEYUP:
                    waiting = False
